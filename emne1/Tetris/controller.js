...
function rotate() {
    ...
    */
    ...
    for (let index = 0; index < board.length; index++) {
        let { colIndex, rowIndex } = drawRotatedBoard(index);
        let newRowIndex;
        for(let i = 0; i<=3;i++) {
            if(colIndex === i) {
                newRowIndex = i;
    ...
    function drawRotatedBoard(index) {
        let rowIndex;
        if (index >= 0 && index <= 3) rowIndex = 0;
        if (index >= 4 && index <= 7) rowIndex = 1;
        if (index >= 8 && index <= 11) rowIndex = 2;
        if (index >= 12 && index <= 16) rowIndex = 3;
        let colIndex;
